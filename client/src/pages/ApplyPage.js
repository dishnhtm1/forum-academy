...
const ApplyPage = () => {
  ...
  useEffect(() => {
    ...
    const createAnimatedElements = () => {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        icon.textContent = ["description", "article", "assignment", "note_alt"][
  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      const applicationData = {
        ...
        lastName:
          formData.personalInfo.fullName?.split(" ").slice(1).join(" ") || "",
      ...
      for (const endpoint of possibleEndpoints) {
        try {
          ...
          if (response.ok) {
            success = true;
            console.log(
              `✅ Application submitted successfully via ${endpoint}`
            );
            break;
          } else {
      ...
      if (!success) {
        // If all endpoints fail, save application locally
        console.log("⚠️ All endpoints failed, saving application locally");
        ...
        createApplicationNotification(newApplication);

        setFormStatus({
          submitted: true,
          error: false,
          message:
            t("applyPage.submitSuccessOffline") ||
            "Application saved locally. It will be submitted when the server is available.",
          loading: false,
        });
      ...
      // Always save application to localStorage for admin dashboard
      const localApplications = JSON.parse(
        localStorage.getItem("pendingApplications") || "[]"
      );
      const newApplication = {
        ...applicationData,
        _id: data._id || `api_${Date.now()}`,
        submittedAt: new Date().toISOString(),
        isLocal: false,
        status: "pending",
      };
  ...
  // Create notification for admin when application is submitted
  const createApplicationNotification = (applicationData) => {
    try {
  ...
  return (
    <div className="apply-page">
      ...
      <section
        ref={sectionRef}
        className={`apply-hero ${isVisible ? "visible" : ""}`}
      >
        ...
        <div className="container">
          <div className="apply-hero-content">
            ...
            <h1 className="apply-hero-title">
              {t("applyPage.hero.title.part1")}{" "}
              <span className="apply-highlight-text">
                {t("applyPage.hero.title.highlight")}
              </span>
            </h1>
              ...
              <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 p-6">
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <span className="text-gray-700 font-medium">
                      {t("applyPage.sidebar.needHelp.faq")}
                    </span>
                  </a>
                </div>
              </div>
              ...
              <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 p-6">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    >
                      <div
                        className={`inline-block px-3 py-1 bg-gradient-to-r ${deadline.color} text-white text-sm font-semibold rounded-lg mb-2`}
                      >
                        {deadline.date}
                      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {testimonials.map((testimonial, index) => (
              <div
                key={index}
                className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl border border-white/20 p-8 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300"
              >
                <div className="flex items-center space-x-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-blue-600 font-medium">
                      {testimonial.program}{" "}
                      {t("applyPage.testimonialsSection.graduateLabel")}
                    </p>
                  </div>
                </div>
