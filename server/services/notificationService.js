...
class NotificationService {
  ...
  static async notifyTeachersStudentProgress(studentId, courseId, progressData) {
    try {
  ...
  static async notifyTeachersHomeworkSubmission(studentId, homeworkId, submissionId) {
    try {
      const teachers = await User.find({ 
        role: { $in: ['teacher', 'admin'] } 
      }).select('_id');
      ...
      console.error('❌ Error notifying teachers of homework submission:', error);
    }
  }
  ...
  static async notifyAdminsApplicationSubmission(applicationId, studentId) {
    try {
      ...
      console.error('❌ Error notifying admins of application submission:', error);
    }
  }
